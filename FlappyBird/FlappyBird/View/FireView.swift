...
class FireView: UIView
{
    ...
    func createYanhua()
    {
        ...
        //发射器中心点
        emitter.emitterPosition = CGPoint(x: self.bounds.size.width / 2.0, y: self.bounds.size.height)
        ...
        //子弹的速度
        bullet.velocity = self.bounds.size.height - 100
        //随机速度范围
    ...
    func setUp() {
        let viewBounds = self.layer.bounds
        ...
        //    自动旋转
        snowflake.spinRange        = CGFloat(Double.pi)        // slow spin
        
        snowflake.contents        =  UIImage(named: "fire.png")?.cgImage
        snowflake.color = UIColor(red: 160.0/255.0, green: 32.0/255.0, blue: 240.0/255.0, alpha: 1).cgColor
