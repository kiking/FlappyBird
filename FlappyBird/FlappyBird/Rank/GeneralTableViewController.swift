...
class GeneralTableViewController: UITableViewController
{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        {
            aab = defaultStand.integer(forKey: UserDefaultKeys.general.gtop1.rawValue)
        }
        else if meters[indexPath.row]=="Top 2"
        {
            aab = defaultStand.integer(forKey: UserDefaultKeys.general.gtop2.rawValue)
        }
        else if meters[indexPath.row]=="Top 3"
        {
            aab = defaultStand.integer(forKey: UserDefaultKeys.general.gtop3.rawValue)
        }
        else if meters[indexPath.row]=="Top 4"
        {
            aab = defaultStand.integer(forKey: UserDefaultKeys.general.gtop4.rawValue)
        }
        else if meters[indexPath.row]=="Top 5"
        {
            aab = defaultStand.integer(forKey: UserDefaultKeys.general.gtop5.rawValue)
        }
